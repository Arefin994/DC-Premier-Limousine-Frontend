...
const VideoAdmin = ({ token, onSuccess }) => {
  ...
  const handleDeleteVideo = async (index) => {
    try {
      ...
      const response = await axios.delete(`${API_BASE_URL}/videos/${videos[index]._id}`, {
        headers: getHeaders(token)
      });
    ...
    <div className="space-y-8">
      ...
      <div className="bg-[#262626] p-6 rounded-lg">
        ...
        <form onSubmit={editingVideo !== null ? handleUpdateVideo : handleAddVideo} className="space-y-4">
          <div>
            <label htmlFor="video-url" className="block text-[#AAAAAA] mb-2">YouTube Video URL</label>
            <input
              type="text"
              id="video-url"
              value={videoForm.url}
            ...
            <p className="text-sm text-[#AAAAAA] mt-1">
              Enter a YouTube video URL (watch or share link). It will be automatically converted to an embed URL.
            </p>
          </div>
